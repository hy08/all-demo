...
```js
//1. 安装依赖 eslint-webpack-plugin、@babel/eslint-parser
//2. 新建eslint配置文件(.eslintrc.js)，确定自己的集成规则，安装相关依赖包
//.eslintrc.js
module.exports = {
...
npm login
// 登录后邮箱会受到验证码
npm publish --otp=邮箱验证码
```

## 优化构建时命令行的显示日志

stats 配置设置命令行显示日志
friendly-errors-webpack-plugin 不支持 webpack5，暂无解决方案
